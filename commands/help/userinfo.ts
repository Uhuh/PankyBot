...
export default {
  ...
  run: async function (message: Message, args: string[], client: PankyBot) {
    let info: string = ""
    const member: GuildMember | undefined = message.mentions.members.first().user.id !== client.user.id ? message.mentions.members.first() : undefined
    const name = member ? ( member.nickname || member.user.username ) : ( message.member.nickname || message.author.username )
